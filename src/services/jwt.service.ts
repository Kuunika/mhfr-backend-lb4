 ...
 */
export const JWT_SECRET = '#MHFR@0192';
...
export class JWTAuthenticationService {
  ...
  constructor(
    ...
    @inject(JWTBindings.SECRET) public jwt_secret: string,
    @inject(PasswordHasherBindings.PASSWORD_HASHER)
    public passwordHasher: PasswordHasher,
  ) {
    this.secret = process.env.JWT_SECRET || this.jwt_secret;
  }
  ...
  async getAccessTokenForUser(credentials: Credentials): Promise<string> {
    ...
    const token = await signAsync(currentUser, this.secret, {
      expiresIn: 3600,
    });
  ...
  async decodeAccessToken(token: string): Promise<UserProfile> {
    const decoded = await verifyAsync(token, this.secret);
